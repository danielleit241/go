...
var validImageExtensions = []string{".jpg", ".jpeg", ".png", ".gif"}
...
func ValidateImage(file *multipart.FileHeader) (bool, error) {
	...
	if !slices.Contains(validImageExtensions, ext) {
		return false, fmt.Errorf("Avatar image must be one of the following types: %s", strings.Join(validImageExtensions, ", "))
	}
